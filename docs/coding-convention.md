...
- 코드 접두사는 feature별 고정: `C` 공통, `A` auth, `M` member, `CA` calendar, `FI` file, `PN` public notice, `NI` internal notice, `RS` resource, `AR` activity record, `EV` event, `F` fee, `P` performance... 새 feature 추가 시 이 문서에 접두사를 등록한다.
