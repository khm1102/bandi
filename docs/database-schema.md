...
파일 바이너리는 DB가 아닌 `FILE_STORAGE_ROOT` 영구 볼륨에 저장한다. DB에는
상대 저장 키와 검증 메타데이터만 둔다. 운영 기본 경로는 `/data/bandi`다.
...
| `storage_key` | VARCHAR(500) | scope 루트 기준 상대 저장 키 |
| `content_type` | VARCHAR(100) | 서버 검증 MIME |
| `size_bytes` | BIGINT | 파일 크기 |
| `sha256_hash` | CHAR(64) | 무결성·중복 확인용 |
| `object_etag` | VARCHAR(100) NULL | 로컬 저장 완료 SHA-256 검증값 |
...
- `FILE_STORAGE_ROOT`는 운영에서 `/data/bandi` 영구 볼륨으로 마운트한다. 루트가 디렉터리가 아니거나 실행 계정이 쓸 수 없으면 애플리케이션은 기동하지 않는다.
- 내부 자료, 활동 인증 사진과 검수 파일은 항상 `private/` 하위에 저장한다. 외부 공시 첨부만 `public/` 하위에 별도 복사할 수 있다. 두 scope 모두 정적 경로로 노출하지 않는다.
- `PRIVATE` 파일의 scope를 직접 바꾸지 않고 공개 승격 시 별도 `PUBLIC` 파일과 `stored_file` 행을 생성한다.
- 객체 키는 `{domain}/{yyyy}/{MM}/{uuid}` 형식으로 서버가 생성하며 원본 파일명, 학번, 멤버 이름과 연락처를 넣지 않는다.
- 저장 경로는 정규화한 뒤 반드시 해당 scope 루트 안에 있어야 한다. 절대 경로와 상위 경로 이탈은 거부한다.
- 배포 시 볼륨의 소유권·권한, 백업과 복구 책임은 운영자가 관리한다. 백업 자동화는 이번 범위에 포함하지 않는다.

### 7.3 업로드·조회·삭제 흐름

1. 1차 구현은 브라우저가 애플리케이션에 multipart 파일을 전송하고 서버가 확장자, 파일 시그니처, 용량과 권한을 검증한다. 클라이언트가 보낸 MIME은 신뢰하지 않고 서버가 판별한다.
2. 재개방 가능한 입력 스트림을 먼저 순회해 실제 크기와 SHA-256을 확정한 뒤 `PENDING` 행을 만든다. 새 스트림은 scope 디렉터리의 임시 파일에 저장하고, 크기를 재검증한 뒤 원자적으로 이동한다.
3. 원자 이동 후 계산한 SHA-256을 `object_etag`에 기록하고 상태를 `READY`로 갱신한 뒤 업무 레코드와 연결한다.
4. 실패하면 임시 파일을 삭제하고 `FAILED`로 전환한다.
5. `PRIVATE`와 `PUBLIC` 파일 모두 Spring Boot가 서비스별 권한 확인 후 직접 스트리밍한다. presigned URL과 302 리다이렉트는 사용하지 않는다.
6. `PUBLIC` 파일은 게시 상태를 확인한 공개 콘텐츠만 읽을 수 있게 하며, 게시 종료 시 공개 연결을 해제한다.
7. 업무 레코드의 논리 삭제만으로 로컬 파일을 즉시 삭제하지 않는다. 모든 참조와 보존 기간을 확인한 정리 작업만 실제 파일을 제거한다.

### 7.4 백엔드 경계와 검증 계약

- `domain.file`이 `stored_file` Mapper와 로컬 파일 저장소를 소유하고 다른 기능에는 `FileService`만 제공한다.
- 자료실, 활동 기록과 소품·장비는 경로나 디렉터리 이름을 직접 참조하지 않는다. 기능 간 호출은 컨벤션대로 Service를 통한다.
- 로컬 저장소 장애 시 업무 레코드를 파일과 연결하지 않고 재시도 가능한 실패로 반환한다.
- `READY`가 아닌 파일은 다운로드·업무 연결·공개 승격에 사용할 수 없다.
- 비공개 다운로드는 요청 멤버의 기능별 접근 권한을 먼저 검증한 뒤 파일을 직접 전송한다.
- 파일 저장 Service 테스트는 정상 저장, 경로 이탈 차단, 원자 이동 실패 보상, 비인가 다운로드, `PENDING` 파일 차단과 공개 승격을 포함한다.
...
| 파일 업로드 | `PENDING` 메타데이터 생성 → 임시 파일 저장·원자 이동 → `READY` 전환 → 업무 레코드 연결 |
...
로컬 파일 이동은 DB 트랜잭션에 완전히 포함할 수 없으므로 `PENDING → READY` 상태와 실패 파일 정리 작업으로 보상한다. DB 롤백만으로 파일이 제거됐다고 가정하지 않는다.
