...
연다. 제목 다음에는 본문을 전체 폭으로 우선 작성하고, 작성과 미리보기는 탭으로 전환하며
동시에 좁게 나누지 않는다. 첨부는 본문 바로 아래에 둔다. 게시 대상은 첨부 다음에 두고,
중요 여부·예약 게시 같은 드문 설정은 접힌 게시 옵션으로 숨긴다. 중복된 상단 게시 버튼은
두지 않고 하단의 하나의 액션 바에서 초안 저장·게시를 처리한다.
