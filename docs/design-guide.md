...
20건 단위 번호형 페이지네이션을 사용한다. 상세는 제목·대상·게시 및 마지막 수정 시각·작성자·본문·
...
**공지 관리** — `/notices/manage`는 읽기 목록과 분리해 초안·예약·게시 중·게시 종료·보관
상태를 검색한다. 각 항목에는 실제 대상 팀, 원 작성자, 최근 수정자·수정 시각을 표시하고,
모바일에서도 가로 표 대신 세로 목록을 사용한다. `/notices/manage/{id}`는 흰색 읽기 표면
아래에 현재 상태에서 가능한 작업만 제공한다. 초안 삭제·게시 종료·보관·초안 복구처럼
결과가 큰 작업은 대상과 결과를 설명하는 확인창을 거치며, 성공 후 서버에서 상세를 다시
조회한다. 읽음 현황은 요약을 먼저 보여 주고 멤버별 목록은 요청할 때 펼친다.

**내부 게시글 공유** — 공지·자료 상세에는 아이콘과 `공유` 텍스트가 있는 44px 이상의 보조
버튼을 둔다. 지원 브라우저에서는 시스템 공유 시트를 열고, 그 외에는 완전한 URL을 복사해
짧은 토스트로 알린다. 작성자·관리 권한자만 제목 공개 링크를 발급·중단할 수 있으며, 첫 발급과
중단 시 카카오·디스코드가 제목 미리보기를 캐시할 수 있음을 확인창으로 알린다. 공유 안내
페이지는 제목과 로그인 안내 외의 내부 정보를 표현하지 않는다.

**목록 페이지네이션** — 공지·자료실·멤버처럼 계속 증가하는 업무 목록은 서버 기반 번호형
페이지네이션을 사용한다. 데스크톱은 이전·최대 5개 번호·다음, 모바일은 이전·현재/전체·다음으로
표시한다. 검색·필터 변경은 1페이지로 돌아가며 URL을 교체하고, 페이지 이동은 방문 기록에
추가해 뒤로 가기에서 조건과 위치를 복원한다. 현재 페이지의 마지막 항목이 사라지면 마지막
유효 페이지로 보정한다. 기준정보처럼 규모가 제한된 팀·기수 목록에는 적용하지 않는다.

**조직 기준정보와 변경 모달** — 기수처럼 짧은 기준정보는 목록 화면의 `기수 관리` 모달에서
추가하고, 삭제·비활성화 같은 파괴적 조작을 함께 노출하지 않는다. 팀·기수 변경은 목록 하단에
폼을 펼치지 않고 선택한 멤버·현재 값·새 값·필수 사유를 보여 주는 모달에서 처리한다. 모바일은
전체 화면 모달을 사용하며, 닫은 뒤에는 원래 변경 버튼으로 포커스를 되돌린다.
...
**저장·검수형 문서 생성 화면** — 문서 입력은 별도 페이지의 단일 세로 흐름으로 구성한다.
상단에는 임시 저장 후 운영진 검수를 요청할 수 있다는 안내와 문서에 반영될 현재 기준값을
먼저 보여준다.
- 데스크톱에서도 우측 설정 패널이나 좁은 sheet를 쓰지 않고 기본 정보, 사진, 장문 내용,
  반복 행 순서로 전체 폭을 사용한다. 모바일은 한 열이며 반복 행만 카드형으로 바꾼다.
- 사진 입력은 클릭과 끌어놓기를 모두 제공하고 선택 즉시 미리보기, 파일명, 교체·삭제를
  제공한다. 허용 형식·용량과 오류를 입력 가까이에 표시한다.
- 반복 참여자 입력은 현재 수와 최대 수를 함께 표시하며 검색 자동 채움 뒤에도 수정할 수
  있어야 한다. 검색 실패 시 직접 입력은 계속 사용할 수 있어야 한다.
- 임시 저장과 검수 요청을 분리한다. 저장 성공은 토스트로만 끝내지 않고 화면 안에서
  HWPX 다운로드, 검수 요청, 새로 작성 행동을 제공한다. 개인정보는 서버의 승인된 활동
  기록·파일 저장소에만 보관하고 브라우저 저장소나 오프라인 큐에는 저장하지 않는다.

### 7.6 인증 화면 (로그인 기능 구현 시 — layoutAuth 내부)
