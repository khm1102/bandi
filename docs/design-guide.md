...
**비저장 문서 생성 화면** — 문서 입력은 별도 페이지의 단일 세로 흐름으로 구성한다.
상단에는 저장되지 않는다는 안내와 문서에 반영될 현재 기준값을 먼저 보여준다.
...
- 생성 성공은 토스트로만 끝내지 않고 화면 안에서 다시 다운로드와 새로 작성 행동을
  제공한다. 개인정보는 브라우저 저장소나 오프라인 큐에 저장하지 않는다.
