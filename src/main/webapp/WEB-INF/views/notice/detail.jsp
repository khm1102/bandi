...
<t:layout title="공지" active="notices" role="${role}">
    <article class="mx-auto max-w-3xl">
        <div class="border-b pb-5">
            ...
            <p class="mt-3 text-xs text-muted-foreground"><c:out value="${notice.publishedByName}"/> · <fmt:formatDate value="${notice.publishStartDttm}" pattern="yyyy.MM.dd HH:mm"/> 게시<c:if test="${not empty notice.updatedDttm}"> · <fmt:formatDate value="${notice.updatedDttm}" pattern="yyyy.MM.dd HH:mm"/> 수정</c:if></p>
        </div>
        <div class="prose prose-slate mt-7 max-w-none text-sm leading-7" data-markdown-content><t:markdown html="${notice.bodyHtml}"/></div>
