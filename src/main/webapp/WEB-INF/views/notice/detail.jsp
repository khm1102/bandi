...
<t:layoutPublic title="${notice.title}" active="notices">
    <article class="mx-auto max-w-3xl">
        <a href="<c:url value='/notices'/>" class="inline-flex min-h-11 items-center text-xs font-bold text-accent-foreground underline-offset-4 hover:underline">공시 목록으로 돌아가기</a>
        <header class="mt-4 border-b pb-6">
            ...
            <h1 class="mt-4 text-3xl font-extrabold leading-tight tracking-tight"><c:out value="${notice.title}"/></h1>
        ...
        <c:if test="${not empty notice.attachments}">
            <section class="border-t py-6" aria-labelledby="noticeAttachmentsTitle">
                <h2 id="noticeAttachmentsTitle" class="text-sm font-bold">첨부파일</h2>
                <ul class="mt-3 divide-y border-y">
                    <c:forEach var="attachment" items="${notice.attachments}">
                        ...
                        <li class="flex flex-col gap-2 p-4 sm:flex-row sm:items-center">
                            ...
                            <a href="<c:out value='${downloadUrl}'/>" class="inline-flex min-h-11 shrink-0 items-center justify-center rounded-md border bg-card px-3 text-xs font-bold hover:bg-secondary">다운로드</a>
                        </li>
                    </c:forEach>
                </ul>
            </section>
        ...
        <footer class="border-t py-6">
            <a href="<c:url value='/notices'/>" class="inline-flex min-h-11 items-center rounded-md border bg-card px-4 text-sm font-bold hover:bg-secondary">공시 목록으로</a>
        </footer>
    </article>
</t:layoutPublic>
