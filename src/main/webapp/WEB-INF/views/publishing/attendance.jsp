...
<t:layout title="출석" active="attendance" role="${role}">
    <t:pageHead title="행사 · 출석 관리" description="회식·MT 등 행사 참가 현황을 실시간으로 확인합니다${canEdit ? ' · 팀장·운영진이 행사를 만들 수 있어요' : ''}">
        <c:if test="${canEdit}">
            <button type="button" data-open-modal="eventModal" class="${btnPrimary}">+ 행사 생성</button>
        </c:if>
    </t:pageHead>

    <t:card title="정기공연 뒤풀이" moreLabel="06/22 · 홍대 회식장소">
        <div class="mb-3.5 flex items-center">
            <b class="text-sm">참가 3명</b>
            <button type="button" class="ml-auto inline-flex h-8 items-center gap-1.5 rounded-md bg-primary px-3 text-xs font-bold text-primary-foreground transition-colors hover:bg-primary-strong hover:text-white">체크인</button>
        </div>
    ...
    <t:modal id="eventModal" title="행사 생성" description="회식·MT 등 새 행사를 만듭니다.">
        <jsp:attribute name="footer">
            ...
            <button type="button" data-action="close-modal" class="${btnPrimary}">행사 생성</button>
        </jsp:attribute>
        <jsp:body>
