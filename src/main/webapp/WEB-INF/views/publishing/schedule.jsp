...
<t:layout title="일정 조율" active="schedule" role="${role}">
    <t:pageHead title="일정 조율" description="부원 가능 시간을 모아 공통 시간을 자동으로 찾습니다">
        <button type="button" data-open-modal="timePickModal" class="${btnPrimary}">시간 입력하기</button>
    </t:pageHead>
    ...
    <div class="grid items-start gap-4 lg:grid-cols-[1.7fr_1fr]">
        <div class="flex flex-col gap-4">
                ...
                <div class="p-5">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <c:if test="${role != 'member'}">
                            <button type="button" class="inline-flex h-8 items-center rounded-md bg-primary px-3 text-xs font-bold text-primary-foreground transition-colors hover:bg-primary-strong hover:text-white">이 시간으로 확정</button>
                        </c:if>
                    </div>
                </div>
            </section>
            ...
            <div class="flex items-center gap-3.5 rounded-lg border bg-card p-5">
                ...
                <div class="min-w-0 flex-1">
                    <b class="text-sm">내 가능 시간을 아직 입력하지 않았어요</b>
                    <p class="text-xs text-muted-foreground">시간 입력하기를 눌러 가능한 칸을 칠해주세요</p>
                </div>
                <button type="button" data-open-modal="timePickModal" class="inline-flex h-8 shrink-0 items-center rounded-md bg-primary px-3 text-xs font-bold text-primary-foreground transition-colors hover:bg-primary-strong hover:text-white">시간 입력</button>
            </div>
        </div>

        <t:card title="응답 현황" moreLabel="4 / 12" flush="true">
            <div class="p-5 pb-3">
                <div class="h-2 overflow-hidden rounded-full bg-secondary"><span class="block h-full rounded-full bg-success" style="width:33%"></span></div>
            </div>
            <div class="flex items-center gap-3 border-b px-5 py-3">
                <span class="flex size-7 items-center justify-center rounded-full bg-primary text-xs font-black text-primary-foreground">${userIni}</span>
                <p class="min-w-0 flex-1 truncate text-sm font-bold">${userName} <span class="text-xs font-semibold text-muted-foreground">(나)</span></p>
                <t:badge tone="neutral">미응답</t:badge>
            </div>
            <div class="flex items-center gap-3 border-b px-5 py-3">
    ...
    <t:modal id="timePickModal" title="내 가능 시간 입력" description="가능한 칸을 눌러 칠하세요. 드래그하면 여러 칸을 한 번에 선택할 수 있어요.">
        <jsp:attribute name="footer">
            ...
            <button type="button" data-action="close-modal" class="${btnPrimary}">가능 시간 저장</button>
        </jsp:attribute>
        <jsp:body>
            ...
            <div class="grid grid-cols-[64px_repeat(5,1fr)] gap-1">
                ...
                <div class="${hcell}">6/26<br><span class="font-semibold">목</span></div>
                <div class="${hcell}">6/27<br><span class="font-semibold">금</span></div>
                <div class="${rcell}">18시</div><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button><button type="button" class="h-8 rounded-md border border-success bg-success"></button><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button>
                <div class="${rcell}">19시</div><button type="button" class="h-8 rounded-md border border-success bg-success"></button><button type="button" class="h-8 rounded-md border border-success bg-success"></button><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button>
                <div class="${rcell}">20시</div><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button>
                <div class="${rcell}">21시</div><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button><button type="button" class="h-8 rounded-md border bg-secondary transition-colors hover:border-primary"></button>
            </div>
