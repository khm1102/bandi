...
<t:layout title="게시판" active="community" role="${role}" scriptPath="community/list">
    <t:pageHead title="커뮤니티 · 게시판" description="공지·자유 의견·질문을 나누는 공간입니다">
        <c:if test="${role != 'member'}">
            <t:button variant="outline" openModal="noticePostModal">공지 작성</t:button>
        </c:if>
        <t:button openModal="postModal">글쓰기</t:button>
    </t:pageHead>

    <div class="mb-4 flex flex-wrap gap-2">
        <t:filterChip group="post" value="전체" label="전체" active="true"/>
        <t:filterChip group="post" value="공지" label="공지"/>
        <t:filterChip group="post" value="자유" label="자유"/>
        <t:filterChip group="post" value="질문" label="질문"/>
    </div>

    <div class="flex flex-col gap-3" data-post-list>
        <div data-post-card data-category="공지" class="rounded-lg border border-primary/40 bg-card p-5 ring-2 ring-ring/10">
            ...
            <div class="mt-3 flex items-center gap-4 text-xs font-bold text-muted-foreground"><button type="button" data-page-action="post-like">♥ <span data-like-count>12</span></button><span>💬 4</span></div>
        </div>
        <div data-post-card data-category="자유" class="rounded-lg border bg-card p-5">
            <div class="mb-2 flex items-center gap-2.5">
            ...
            <div class="mt-3 flex items-center gap-4 text-xs font-bold text-muted-foreground"><button type="button" data-page-action="post-like">♥ <span data-like-count>7</span></button><span>💬 9</span></div>
        </div>
        <div data-post-card data-category="질문" class="rounded-lg border bg-card p-5">
            <div class="mb-2 flex items-center gap-2.5">
            ...
            <div class="mt-3 flex items-center gap-4 text-xs font-bold text-muted-foreground"><button type="button" data-page-action="post-like">♥ <span data-like-count>2</span></button><span>💬 3</span></div>
        </div>
        <div data-post-card data-category="자유" class="rounded-lg border bg-card p-5">
            <div class="mb-2 flex items-center gap-2.5">
            ...
            <div class="mt-3 flex items-center gap-4 text-xs font-bold text-muted-foreground"><button type="button" data-page-action="post-like">♥ <span data-like-count>15</span></button><span>💬 22</span></div>
        </div>
    </div>
    ...
    <t:modal id="postModal" title="글쓰기" description="부원들과 자유롭게 의견을 나눠보세요.">
        <jsp:attribute name="footer">
            ...
            <t:button pageAction="post-add">등록</t:button>
        </jsp:attribute>
        <jsp:body>
    ...
    <t:modal id="noticePostModal" title="공지 작성" description="공지는 게시판 상단에 고정되고 전체 알림이 발송됩니다.">
        <jsp:attribute name="footer">
            <t:button variant="outline" action="close-modal">취소</t:button>
            <t:button pageAction="post-notice">공지 등록</t:button>
        </jsp:attribute>
        <jsp:body>
            <div class="flex flex-col gap-3">
