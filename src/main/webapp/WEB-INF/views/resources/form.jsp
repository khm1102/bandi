        ...
        <section class="space-y-5" aria-labelledby="resourceBodyHeading">
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 border-t pt-6">
                ...
                <div class="flex rounded-md border bg-card p-1" role="tablist" aria-label="본문 보기 방식">
                    <button type="button" class="min-h-9 rounded-sm bg-secondary px-3 text-sm font-bold" role="tab" aria-selected="true" aria-controls="resourceEditorPanel" id="resourceWriteTab" data-resource-tab="write">작성</button>
                    <button type="button" class="min-h-9 rounded-sm px-3 text-sm font-bold text-muted-foreground" role="tab" aria-selected="false" aria-controls="resourcePreviewPanel" id="resourcePreviewTab" data-resource-tab="preview">미리보기</button>
                </div>
            </div>
            ...
            <div id="resourceEditorPanel" class="mt-3" role="tabpanel" aria-labelledby="resourceWriteTab" data-resource-panel="write">
                <div class="flex flex-wrap gap-1 rounded-t-md border border-b-0 bg-secondary/50 p-2" aria-label="Markdown 서식 도구">
                    <button type="button" class="flex min-h-9 min-w-9 items-center justify-center rounded-sm px-2 text-sm font-extrabold hover:bg-card" data-resource-markdown-action="bold" aria-label="굵게" title="굵게">B</button>
                    <button type="button" class="flex min-h-9 min-w-9 items-center justify-center rounded-sm px-2 text-sm italic hover:bg-card" data-resource-markdown-action="italic" aria-label="기울임" title="기울임">I</button>
                    <span class="mx-1 h-7 self-center border-l" aria-hidden="true"></span>
                    <button type="button" class="min-h-9 rounded-sm px-2 text-xs font-bold hover:bg-card" data-resource-markdown-action="heading-one">H1</button>
                    <button type="button" class="min-h-9 rounded-sm px-2 text-xs font-bold hover:bg-card" data-resource-markdown-action="heading-two">H2</button>
                    <button type="button" class="min-h-9 rounded-sm px-2 text-xs font-bold hover:bg-card" data-resource-markdown-action="list">목록</button>
                    <button type="button" class="min-h-9 rounded-sm px-2 text-xs font-bold hover:bg-card" data-resource-markdown-action="quote">인용</button>
                    <button type="button" class="min-h-9 rounded-sm px-2 text-xs font-bold hover:bg-card" data-resource-markdown-action="code">코드</button>
                    <button type="button" class="min-h-9 rounded-sm px-2 text-xs font-bold hover:bg-card" data-resource-markdown-action="link">링크</button>
                    <button type="button" class="min-h-9 rounded-sm px-2 text-xs font-bold hover:bg-card" data-resource-markdown-action="table">표</button>
                    <button type="button" class="min-h-9 rounded-sm px-2 text-xs font-bold hover:bg-card" data-resource-markdown-action="image">이미지</button>
                </div>
                <textarea id="resourceBody" class="min-h-[30rem] w-full rounded-b-md border border-input bg-card px-4 py-4 font-mono text-sm leading-6 placeholder:text-muted-foreground focus:border-ring focus:outline-none focus:ring-2 focus:ring-ring/20" required placeholder="# 자료 제목\n\n자료 내용을 Markdown으로 작성하세요. 단독 HTTPS URL은 저장 후 링크 카드로 보여요." data-resource-body></textarea>
