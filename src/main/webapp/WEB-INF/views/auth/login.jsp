...
<t:layoutAuth title="학교 계정 로그인" scriptPath="auth/login">
    ...
    <div class="mt-5 flex flex-col gap-3" role="form" aria-label="학교 계정 로그인" data-auth-form data-dashboard-url="${dashboardUrl}">
        <div>
            <label class="${label}" for="schoolId">학교 포털 아이디</label>
            <input class="${input}" id="schoolId" type="text" autocomplete="username" placeholder="학교 포털 아이디" required maxlength="50" aria-describedby="loginTrust authError">
        </div>
        <div>
            <label class="${label}" for="schoolPassword">학교 포털 비밀번호</label>
            <input class="${input}" id="schoolPassword" type="password" autocomplete="current-password" placeholder="학교 포털 비밀번호" required maxlength="100" aria-describedby="loginTrust authError">
        </div>
        <c:choose>
        ...
        <t:button cssClass="mt-1 w-full" pageAction="auth-submit">학교 계정으로 로그인</t:button>
    </div>
    ...
    <div id="loginTrust" class="mt-5 border-t pt-4 text-xs leading-5 text-muted-foreground">
        ...
        <p class="mt-2 rounded-md bg-accent/60 px-3 py-2 text-accent-foreground">개발 목업에서는 입력값을 학교로 전송하지 않고 로그인 이후 화면만 확인합니다.</p>
    </div>
