<%@ tag description="관리자 셸 사이드바 — 그룹형 메뉴, 역할별 노출, 활성 상태, 프로필 관리 (역할 조건은 기존 정책 그대로)" pageEncoding="UTF-8" trimDirectiveWhitespaces="true" %>
...
<c:set var="navOn" value=" bg-primary font-bold text-primary-foreground"/>
<c:set var="groupLabel" value="px-3 pb-1 pt-4 text-xs font-bold text-sidebar-muted"/>
<aside id="mainNavigation"
    ...
    <a href="<c:url value='/dashboard'/>" class="flex min-h-14 items-center gap-2.5 border-b border-sidebar-border px-2 pb-4">
        ...
        <span>
            <b class="block text-sm font-extrabold text-white">반디</b>
            <span class="block text-xs font-semibold text-sidebar-muted">연극 동아리</span>
        </span>
    </a>
    <nav class="mt-1 flex flex-col gap-1" aria-label="주 메뉴">
        <a href="<c:url value='/dashboard'/>" class="mt-3 ${navBase} ${active == 'dashboard' ? navOn : navOff}" aria-current="${active == 'dashboard' ? 'page' : 'false'}">
        ...
        <c:if test="${role == 'admin'}">
            ...
            <a href="<c:url value='/showops'/>" class="${navBase} ${active == 'showops' ? navOn : navOff}" aria-current="${active == 'showops' ? 'page' : 'false'}">
                <svg class="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 7V5a2 2 0 0 1 2-2h2M17 3h2a2 2 0 0 1 2 2v2M21 17v2a2 2 0 0 1-2 2h-2M7 21H5a2 2 0 0 1-2-2v-2M7 12h10"/></svg>공연 당일 입장
            </a>
    ...
    <div class="mt-auto flex flex-col gap-3 border-t border-sidebar-border px-2 pt-3">
        <div class="flex min-w-0 items-center gap-2.5" data-session-profile aria-busy="true">
            <span class="flex size-8 shrink-0 items-center justify-center rounded-full bg-primary text-xs font-extrabold text-primary-foreground" data-session-initial>·</span>
        ...
        <form action="<c:url value='/logout'/>" method="post">
            ...
            <button type="submit" class="inline-flex min-h-11 w-full items-center justify-center whitespace-nowrap rounded-md border border-sidebar-border px-3 text-xs font-bold text-sidebar-muted transition-colors hover:bg-sidebar-accent hover:text-white">로그아웃</button>
        </form>
    </div>
</aside>
